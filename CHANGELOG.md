...
- **Collapse engine side effects into a single `actor.subscribe` dispatcher** — closes the architectural BACKLOG item and the three open bug-class items it umbrellas (stale `ctx` in `startTimer`, stale `sequence`/`level` in `continueGame`, double-fire of `saveHighScore`/`saveDailyResult`). `useGameEngine.ts` previously interleaved audio / haptics / persistence / analytics with machine wiring across `setTimeout` callbacks, `setInterval` ticks, `useEffect` watchers, and imperative calls inside action handlers — every path could read stale closure state. Now `useMachine` is destructured as `[state, send, actorRef]`, and a single `actor.subscribe` callback dispatches all state-driven effects keyed on `(prev, next)` of `snapshot.value`. Per-button `noteOn`/`noteOff` and `buttonPress` haptic stay inline in touch/release handlers (sub-frame audio latency requirement). Refs that existed only to compensate for stale-closure problems (`contextRef`, `scoreRef`, `sideEffectsFiredRef`) and the `handleGameOverSideEffects` function are deleted. **Why `actor.subscribe` rather than `useEffect` on `state.value`**: subscribe fires synchronously when `send()` causes a transition, so a callback running inside a fake-timer tick can dispatch follow-up effects (visual sequence scheduling, advancement beat) that schedule further timers within the same `advanceTimersByTime` window — preserving the pattern the existing 130-line hook test suite relies on. New `src/hooks/__tests__/gameEngineMachine.test.ts` (29 machine-only tests using `createActor`) locks the machine contract independently of the React layer; new regression tests in `useGameEngine.bugs.test.ts` cover timer-expiry single-fire and post-continue same-score persistence. 162 tests green; pre-existing 3-error tsc baseline unchanged. Manual iOS device validation (audio path) still required before merging to `develop`.
- **Symmetric input-lock guard on button release** — adjacent closure-class bug. `handleButtonTouch` checks `state.value !== "waiting"` at touch time and `handleButtonRelease` rechecked only `state.value` at release time. A press during `showing` (rejected silently — no `noteOn` / haptic / `activeButton` / `inputLocked`) followed by a release after `SEQUENCE_DONE` propagates would still validate and dispatch `CORRECT_INPUT` — input registered, no audio, no flash, reproducible by tapping immediately on the first color of a sequence. Fix: release path now requires `inputLocked.current === true`. Same fix in `previewPadRelease`. Two existing `isNewHighScore` tests in `useGameEngine.test.ts` were silently relying on this bug to reach gameover via `advanceTimersByTime(3000)` (which lands inside round 2's `showing` rather than `waiting`); bumped to 4000ms with explicit `gameState` assertions. New regression test in `useGameEngine.bugs.test.ts` reproduces the press-straddle directly.
- **Verified: stale closures in `GameScreen.tsx` long-lived effects** — already addressed by prior work. Pending-action handler reads `resetGameRef` / `handleContinueRef` / `clearPendingActionRef` at call time; the gameover-transition effect (`GameScreen.tsx:396-485`) ref-wraps every captured identity (`analytics`, `haptics`, `router`, `checkIsHighScore`, `addHighScore`, `getRank`, `rescheduleAfterGameOver`, `incrementGamesPlayed`, `playHighScoreJingle`, `playGameOverJingle`, `navigateToGameOver`).
- **Verified: `useGameOverStore()` whole-store subscription** — already addressed. `src/app/game-over.tsx:131-141` uses per-field selectors for all 10 fields with an inline source comment explaining the rationale.
- **Async `onPress` handlers void-wrapped** — `handleStartGame` (both `GameScreen.tsx` call sites), `handleShare` in `game-over.tsx`, and the four IAP / restore-purchases handlers in `settings.tsx`. `onPress` expects `() => void`; passing `async` returns a `Promise` that is silently dropped along with any rejection.

### Fix (vitals)

- **Play Console 1.0.1 vitals — deprecated edge-to-edge APIs + large-screen resizability** — Android vitals flagged two store-visible items on the 1.0.1 build. (1) **Edge-to-edge**: stripped the deprecated `backgroundColor={activeTheme.backgroundColor}` prop from the four `expo-status-bar` `<StatusBar>` call sites (`src/app/{leaderboard,achievements,stats}.tsx`, `src/screens/GameScreen.tsx`); under `react-native-edge-to-edge` the prop resolves to `Window.setStatusBarColor` which Android 15 (API 35) deprecates, and the screen's root `View` already paints `activeTheme.backgroundColor` so the bar shows through correctly. The `style` prop (light/dark icon foreground) is unaffected and stays. `react-native-edge-to-edge@1.6.0` plugin already configured at `app.config.ts:82–90` (`parentTheme: "Light"`, `enforceNavigationBarContrast: false`); SDK 55 defaults `edgeToEdgeEnabled` true, no `expo-build-properties` change needed. (2) **Resizability**: no code change — the v1.1.0 tablet pass already addressed the structural cause (manifest now generates `android:screenOrientation="unspecified"` from `app.config.ts:19` `orientation: "default"`, no `resizeableActivity="false"`, runtime portrait-lock for phones via `OrientationLockProvider` only at the 600dp shortest-side breakpoint, tablets unlock to use the existing tablet-portrait + tablet-landscape `GameScreen` compositions). 1.0.1 was flagged because it shipped `orientation: "portrait"` (manifest hard-lock) before the tablet work landed; 1.1.0 ships clean. Verified via `grep -E "screenOrientation|resizeableActivity" android/app/src/main/AndroidManifest.xml`. 130 tests green, 3-error tsc baseline unchanged.
...
- **Android rapid-tap crackle test — restored v1.1.0 pooling on both platforms** — Gameplay pads now use the same 4-voice always-running oscillator pool on Android and iOS (`padOscPoolRef`, one gate per `POOL_FREQS` entry). Android `noteOn` / `noteOff` no longer create per-press `AudioBufferSourceNode`s or use the 12 ms warm lookahead path during user taps, and the master graph routes directly to destination on both platforms for an apples-to-apples device test. Android buffer helpers remain for sequence playback / A/B experiments. `src/hooks/useAudioTones.tsx`, `src/utils/audio/padOscPool.ts`, `src/utils/audio/androidPadBuffer.ts`, `docs/AUDIO-ARCHITECTURE.md`.
...
- **Audio pad utils split: `padBufferVoice.ts` → `padShared.ts` + `padOscPool.ts` + `androidPadBuffer.ts`** — split the shared constants, cross-platform gameplay pad pool, and Android buffer helpers. Now: (1) [`src/utils/audio/padShared.ts`](src/utils/audio/padShared.ts) holds cross-platform constants (`POOL_FREQS`, `ONE_SHOT_ATTACK_S`, `PAD_RELEASE_S`, `SUSTAIN_PAD_PEAK`, `DEFAULT_PAD_TARGET_GAIN`, `SEQUENCE_LOOKAHEAD_S`, `SEQUENCE_ATTACK_S`, `SEQ_RELEASE_S`) plus `scheduleLinearPadRelease`. (2) [`src/utils/audio/padOscPool.ts`](src/utils/audio/padOscPool.ts) exports `PadOscPool` / `PadPoolVoice` types and `buildPadOscPool` / `teardownPadOscPool` / `setPadOscPoolWave` / `silencePadOscPool` / `padOscPoolNoteOn` / `padOscPoolNoteOff` — pure functions over an explicit pool argument, no React state. (3) [`src/utils/audio/androidPadBuffer.ts`](src/utils/audio/androidPadBuffer.ts) keeps Android buffer code and the old warm/cold lookahead constants for sequence-path tuning / A/B comparison experiments. The hook ([`src/hooks/useAudioTones.tsx`](src/hooks/useAudioTones.tsx)) now reads as orchestration: gameplay pad branches call into `padOscPool`, while sequence/jingle/preview scheduling remains local. Sole external consumer (`useAudioTones.tsx`) updated; no other imports of the old `padBufferVoice` module existed in the codebase. Docs synced ([docs/AUDIO-ARCHITECTURE.md](docs/AUDIO-ARCHITECTURE.md) → key-files table now lists all four modules with roles).

- **`padBufferVoice` Hann cleanup** — Removed unused `scheduleHannPadAttack` / `scheduleHannPadRelease`, `PAD_ENVELOPE_CURVE_POINTS`, and attack-duration constants that only served Hann; `getPadBufferAttackParams` now returns `{ attackLookaheadS }` only (see [src/utils/audio/androidPadBuffer.ts](src/utils/audio/androidPadBuffer.ts)).
