...
- [x] **State machine: `startTimer` interval reads stale `ctx` via JS closure**
      Resolved by the architectural side-effect consolidation. The timer's `setInterval` no longer calls `handleGameOverSideEffects` directly — it just sends `TIMER_EXPIRED`, and the consolidated state-watching dispatcher (subscribed to the actor) reads live `state.context` on the gameover transition. Regression locked in by `useGameEngine.bugs.test.ts` › "timer expiry persists the live score exactly once".

- [x] **State machine: `continueGame` captures `ctx.sequence` / `ctx.level` via closure before `addTimeout`**
      Resolved by the architectural side-effect consolidation. `continueGame` now sends `CONTINUE` and the consolidated dispatcher reads live `snapshot.context.sequence` / `.level` on the `starting` entry — no synchronous snapshot capture. The previous "schedule replay in nested addTimeouts" pattern is gone.

- [x] **`handleGameOverSideEffects` can double-fire `saveHighScore` / `saveDailyResult`**
      Resolved by the architectural side-effect consolidation. The function is deleted; gameover-entry persistence runs inside the actor.subscribe dispatcher, gated by `prev !== next` so any number of gameover-routed events in the same transition fire the side effects exactly once. `saveHighScore` is gated on `live.isNewHighScore`, `recordGameResult` on `!live.continuedThisGame`. Regression locked in by `useGameEngine.bugs.test.ts` › "saveString(highScoreKey) fires at most once" + "does not re-persist highScore on a same-score gameover after continue".

- [x] **Ad serving dies for rest of session on single ERROR event**
      Verified already implemented. `useAds.ts:156-165` (interstitial) and `:195-205` (rewarded) schedule a 30s `setTimeout` reload on `AdEventType.ERROR`, with cancel-on-LOADED and cancel-on-unmount semantics via `interstitialRetryRef` / `rewardedRetryRef`. Constant: `AD_RETRY_DELAY_MS = 30 * 1000`. Industry best practice would upgrade to exponential backoff for network errors specifically (1/2/4/8/16/32s, cap 60s, ~5 attempts) with a longer fixed delay for `ERROR_CODE_NO_FILL`, but the current fixed 30s retry is functionally correct and not on fire — defer the optimization until eCPM data shows fill loss.

- [x] **`consentReady` is exported from `useAds` but nothing consumes it**
      Stale — verified `consentReady` is not actually exported. The hook's `UseAdsReturn` type at `useAds.ts:50-57` includes only `showInterstitial`, `showRewarded`, `rewardedReady`, `incrementGamesPlayed`, `incrementSessionCount`, `adShownThisSession`. NPA defaults to `true` (EEA-safe) until consent resolves, then is updated based on TCF purposes 3+4. No code smell to fix.
...
- [x] **Banner ad on `/leaderboard` (first non-intrusive banner placement)**
      Shipped. `AnchoredBanner` component (`src/components/AnchoredBanner.tsx`) renders `BannerAdSize.ANCHORED_ADAPTIVE_BANNER` with a reserved-height container (60dp), `useSafeAreaInsets` bottom inset, and a 16dp gap from the score table. Gated by `!removeAds`. Mounted in `src/app/leaderboard.tsx` at the bottom of the screen. Emits `ad_shown` PostHog event with `{ type: "banner", placement: "leaderboard" }` once per mount.

- [x] **"Tired of ads?" conversion prompt after Nth interstitial**
      Shipped. `useTiredOfAdsPrompt` hook (`src/hooks/useTiredOfAdsPrompt.ts`) gates on `ADS_LIFETIME_INTERSTITIALS_SHOWN >= 3` AND `IAP_TIRED_OF_ADS_PROMPT_SHOWN !== "true"` AND `!removeAds`. Lifetime counter increments in `useAds.showInterstitial` after `.show()` succeeds. Triggered from `/game-over` mount effect (mutually exclusive with PostPB — PB has priority on new-high-score games). New `TiredOfAdsPrompt` component reuses `PostPBPrompt`'s `ModalOverlay` + `PressableScale` chrome; sparkles icon, accent-colored CTA. Copy: "Enjoying Eco Mi? Play ad-free." / "One-time purchase. No subscriptions." / "Remove Ads" (en/es/pt). One-shot for v1; persistent flag set on dismiss/convert so a network failure doesn't burn the conversion window. Analytics: `tired_of_ads_prompt_{shown,dismissed,converted}`.

- [x] **Rewarded-continue → IAP handoff after multiple watches in a session**
      Shipped. New `sessionAdStore` (Zustand, session-scoped — resets on cold launch) tracks `rewardedWatchedThisSession` (incremented in `useAds` on `EARNED_REWARD`), `tiredOfAdsShownThisSession`, and `swappedCtaGameOversThisSession`. `shouldSwapRemoveAdsCta` returns `true` when watched >= 2, tired-of-ads has not fired this session, and the swap has appeared on fewer than 4 game-overs. On `/game-over`, when the swap is active, the passive "Remove Ads" text link becomes a filled `PressableScale` button using `theme.accentColor` + `primaryForegroundColor` with copy `"Skip the ads"` (en/es/pt) and direct `purchaseRemoveAds()`. Cap of 4 game-overs/session prevents prompt fatigue. Analytics: `remove_ads_cta_swapped` (one-time per session), `remove_ads_cta_tap` `{ variant: "passive" | "swapped" }`.

- [ ] **"Great session" review prompt trigger (non-PB engagement signal)**
      Today `useStoreReview` only fires on `new_high_score` and daily streak milestones (3/7-day). PB-on-game-2 is too early — the user hasn't invested yet — and most sessions don't end in a PB at all, so the prompt budget (Apple/Google cap: 3/365 days) is poorly spent. Add a "great session" trigger: long elapsed session time + multiple games played in the session + no rage-quit signal (e.g. didn't quit immediately after a low-score loss). Likely thresholds to tune: session ≥ ~5 min, ≥ 3 games played this session, last game wasn't a sub-level-3 immediate quit. Fires from `/game-over` mount effect, mutually exclusive with the existing PB and streak triggers. Goal is to seed first reviews from engaged-but-not-PB users since the app currently has zero reviews/ratings, which kills ASO discoverability. Add a new `reviewTrigger` value (`"great_session"`) so analytics can A/B vs the PB trigger.

- [ ] **Post-v1.2.0 monetization conversion review**
      Once v1.2.0 has been live for ~7 days with non-trivial install volume, pull PostHog event counts to evaluate the conversion levers shipped in this release. Events to query: `ad_shown` filtered to `type: "banner"` and `placement: "leaderboard"` (banner impression volume + fill rate as a proxy for AdMob health); `tired_of_ads_prompt_shown` / `_dismissed` / `_converted` (one-shot modal funnel — conversion = converted/shown, dismissal = dismissed/shown); `remove_ads_cta_swapped` (sessions where the rewarded→IAP swap fired) and `remove_ads_cta_tap` `{ variant }` (tap-rate lift of swapped vs passive). Decision points the data should drive: (a) whether to layer in the deferred re-prompt cadence at lifetime interstitial #8 then #15 (industry guidance is +12–18% lifetime ARPDAU vs one-shot, but only ship if the one-shot data justifies); (b) whether to upgrade `useAds` ERROR retry from fixed-30s to exponential backoff (1/2/4/8/16/32s, cap 60s, ~5 attempts) for `ERROR_CODE_NETWORK_ERROR` specifically — only worth doing if eCPM data shows fill loss; (c) whether to extend the banner placement to `/stats` and `/achievements` (secondary candidates with similar dwell-time / read-only profile). Cross-reference with AdMob console eCPM by placement to validate conversion vs revenue tradeoffs (rewarded IAP conversion cannibalizes ~3–5% of rewarded eCPM lifetime per converted user but is +8–15× LTV — the math should still favor conversion).
...
- [x] **Collapse game-engine side effects into a single state-watching effect (architectural)**
      Done. `useGameEngine.ts` now subscribes to the actor (`useMachine` returns `[state, send, actorRef]`) and dispatches all state-driven side effects from one `actor.subscribe` callback keyed on `(prev, next)` of `snapshot.value`. Per-button `noteOn`/`noteOff` and `buttonPress` haptic remain inline in touch/release handlers (sub-frame audio latency requirement). Refs that existed only to compensate for stale-closure problems (`contextRef`, `scoreRef`, `sideEffectsFiredRef`, `handleGameOverSideEffects`) were deleted. Behavioral parity verified against the existing 130-line hook test suite plus 29 new machine-only unit tests in `gameEngineMachine.test.ts` and the regression tests for the three named bug-class items. `actor.subscribe` (rather than a `useEffect` on `state.value`) is the load-bearing detail: it fires synchronously when `send()` causes a transition, preserving the existing tests' "schedule next phase inside the addTimeout that fired this one" choreography.

- [x] **Stale closures in `GameScreen.tsx` long-lived effects**
      Verified resolved. The pending-action handler reads `resetGameRef` / `handleContinueRef` / `clearPendingActionRef` at call time. The gameover-transition effect ref-wraps every captured identity (`analyticsRef`, `hapticsRef`, `routerRef`, `checkIsHighScoreRef`, `addHighScoreRef`, `getRankRef`, `rescheduleAfterGameOverRef`, `incrementGamesPlayedRef`, `playHighScoreJingleRef`, `playGameOverJingleRef`, `navigateToGameOverRef`) updated each render. See `GameScreen.tsx:396-485`.

- [x] **`useGameOverStore()` whole-store subscription**
      Verified resolved at `src/app/game-over.tsx:131-141`. Per-field selectors in place for `score`, `level`, `highScore`, `previousHighScore`, `isNewHighScore`, `mode`, `showRemoveAds`, `showContinue`, `sessionTime`, `needsInitials`. Comment in source explains the rationale.

- [x] **Async `onPress` handlers without `void` wrapping**
      Wrapped: `handleStartGame` (both call sites in `GameScreen.tsx`), `handleShare` in `game-over.tsx`, and the four IAP / restore-purchases handlers in `settings.tsx`. Pattern: `onPress={() => { void handleX() }}` for refs / direct calls; for inline-async bodies, `onPress={() => { void (async () => { ... })() }}`.
...
- [ ] **Better tablet layout for Leaderboard and Stats screens**
      The current tablet treatment for `src/app/leaderboard.tsx` and `src/app/stats.tsx` just clamps content to `maxWidth: 700` and centers it. On a 13" iPad Pro this leaves ~60–70% of the screen as empty void below a small block of content (verified via screenshots: leaderboard shows 5 rows then dead space; stats shows a 2×3 card grid in the upper third with the rest of the screen empty; daily/empty-state shows a tiny trophy icon floating in a sea of background). Options to explore: (a) **Side-by-side layout in landscape** — mode picker / category list on the left, scores or stat detail on the right; (b) **Larger, denser content** — bigger row heights, larger fonts, more rows visible, and/or expand stats grid to 3×N with richer cards (per-mode breakdowns, recent activity, charts); (c) **Inline detail pane** — tapping a row on the leaderboard reveals a detail panel beside it (date, level, mode-specific stats) instead of being a flat list; (d) **Charts on Stats** — add a 7/30-day score sparkline, mode-distribution donut, or streak calendar to fill the dead vertical space with signal, not padding. Should respect `isTablet` from `useBreakpoints()` and ideally distinguish portrait vs landscape (the `useGameBoardMetrics` pattern from GameScreen is a good reference). Audit `achievements.tsx` at the same time — same `maxWidth: 700` pattern likely produces the same dead-space problem on 13" iPad. (`game-over.tsx` reads fine on iPad — header up top, stat grid in the middle, CTAs anchored to the bottom — leave it alone.)
  - Ref: existing tablet entry above; affected files: `src/app/leaderboard.tsx`, `src/app/stats.tsx`, `src/components/HighScoreTable.tsx`, `src/utils/layoutBreakpoints.ts`

- [ ] **Better tablet layout for Settings screen (form sheet or centered content)**
      `src/app/settings.tsx` has *zero* tablet handling — opposite problem from leaderboard/stats. On a 13" iPad Pro the rows span the full ~1100pt width, so labels (e.g. "Vibration feedback", "Daily challenge reminders") sit on the far left while their toggles sit ~900pt away on the far right. Volume slider is the same — a single ~1000pt-wide track. Looks broken, not just unpolished. Two viable approaches: (a) **Expo Router form sheet** — change the route's presentation to `presentation: "formSheet"` (or `"modal"`) on tablet so iOS shows it as a centered floating card with the native settings-app feel. Cleanest "this app understands iPad" signal but changes navigation semantics (modal dismissal gesture, no pushed back-button). Need to verify behavior on Android tablets. (b) **Centered content with `maxWidth` clamp** — same pattern as leaderboard/stats but tighter (~560–620pt feels right for a settings list). Less platform-native but consistent with the rest of the app and works identically on Android. Either way, audit row heights and the Remove Ads / Restore Purchases CTAs — the buttons currently span full width which also looks odd on tablet. Lean toward (a) for iPad since it matches Apple's settings idiom; fall back to (b) on Android.
  - Affected files: `src/app/settings.tsx`, `src/app/_layout.tsx` (route presentation config if going the form-sheet route)
