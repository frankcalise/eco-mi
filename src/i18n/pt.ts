const pt = {
  common: {
    ok: "OK",
    cancel: "Cancelar",
    back: "Voltar",
    done: "Pronto",
    on: "Sim",
    off: "Não",
    dismiss: "Dispensar",
  },
  game: {
    title: "Eco Mi",
    subtitle: "Jogo de Memória",
    startGame: "Iniciar Jogo",
    reset: "Reiniciar",
    endGame: "Encerrar",
    sound: "Som",
    playAgain: "Jogar Novamente",
    share: "Compartilhar",
    gameOver: "Fim de Jogo",
    newHighScore: "Novo Recorde!",
    score: "Pontos",
    level: "Nível",
    best: "Melhor",
    time: "Tempo",
    watchSequence: "Observe a sequência...",
    repeatSequence: "Repita a sequência!",
    timeGained: "Bom trabalho! +{{delta}}s",
    timePenalty: "Ops, tente novamente! -{{delta}}s",
    pressStart: "Pressione Iniciar Jogo para começar!",
    continue: "Assistir Anúncio para Continuar",
    removeAds: "Remover Anúncios",
    restorePurchases: "Restaurar Compras",
    restoreSuccess: "Compras restauradas!",
    restoreFailed: "Nada para restaurar",
    home: "Início",
    mainMenu: "Menu Principal",
    leaderboard: "Classificação",
    shareMessage:
      "Cheguei ao Nível {{level}} com {{score}} pontos no modo {{mode}} no Eco Mi! Consegue me superar?\n\nhttps://frankcalise.github.io/eco-mi/download.html",
    timeRemaining: "{{time}}s",
    sequencesCompleted: "Sequências: {{count}}",
    unlockSound: "Desbloquear Som ({{name}})",
    unlockTheme: "Desbloquear Tema ({{name}})",
    scoreLbl: "PONTOS",
    levelLbl: "NVL",
    ptsLvl: "PTS - NVL",
    modes: {
      classic: "Clássico",
      daily: "Diário",
      timed: "Cronometrado",
      reverse: "Reverso",
      chaos: "Caos",
    },
    settings: "Configurações",
    modeSelect: "Selecionar Modo",
    modeDescriptions: {
      classic: "Lembre e repita a sequência",
      daily: "Um novo desafio todo dia",
      timed: "Complete sequências em 60 segundos",
      reverse: "Repita a sequência ao contrário",
      chaos: "Os botões mudam a cada rodada",
    },
    soundDisabledHint: "Ative o som para ouvir",
    soundPreviewHint: "Toque em um pacote para ouvi-lo",
    soundPack: "Pacote de Som",
    theme: "Tema",
    soundToggle: "Som",
    newHighScoreEntry: "NOVO RECORDE!",
    enterInitials: "Digite suas iniciais",
    initialsPrompt: "Como devemos te chamar?",
    saveInitials: "Salvar",
    skipInitials: "Pular",
    done: "PRONTO",
    highScores: "Melhores Pontuações",
    rank: "POS",
    initials: "NOME",
    emptyInitials: "---",
    emptyScore: "----",
    emptyLeaderboard: "Jogue uma partida para conseguir sua primeira pontuação!",
    streakAtRisk: "🔥 Sequência de {{count}} dias. Jogue Daily para manter!",
    pbDelta: "+{{delta}} acima da sua melhor marca anterior!",
    nearMiss: "Quase lá! Apenas {{delta}} do seu melhor!",
    shareCardWatermark: "Jogue Eco Mi — Consegue superar isso?",
    mode: "Modo",
  },
  stats: {
    title: "Estatísticas",
    gamesPlayed: "Partidas Jogadas",
    bestScore: "Melhor Pontuação",
    averageScore: "Pontuação Média",
    totalScore: "Pontuação Total",
    currentStreak: "Sequência Atual",
    longestStreak: "Melhor Sequência",
    dayUnit: "dia",
    daysUnit: "dias",
    emptyTitle: "Sem jogos ainda",
    emptyBody: "Jogue sua primeira partida para ver suas estatísticas aqui",
    playNow: "Jogar Agora",
  },
  leaderboard: {
    emptyTitle: "Ainda sem pontuações",
  },
  achievements: {
    title: "Conquistas",
    unlocked: "Desbloqueado!",
    first_game: "Primeiros Passos",
    first_game_desc: "Complete sua primeira partida",
    score_100: "Três Dígitos",
    score_100_desc: "Consiga 100 pontos",
    score_500: "Grande Jogador",
    score_500_desc: "Consiga 500 pontos",
    score_1000: "Mestre da Memória",
    score_1000_desc: "Consiga 1000 pontos",
    level_5: "Aquecendo",
    level_5_desc: "Alcance o nível 5",
    level_10: "Pegando Fogo",
    level_10_desc: "Alcance o nível 10",
    level_12: "Ficando sério",
    level_12_desc: "Alcance o nível 12",
    level_15: "Imparável",
    level_15_desc: "Alcance o nível 15",
    level_20: "Lendário",
    level_20_desc: "Alcance o nível 20",
    streak_3: "Consistente",
    streak_3_desc: "Sequência de 3 dias no desafio diário",
    streak_7: "Dedicado",
    streak_7_desc: "Sequência de 7 dias no desafio diário",
    streak_14: "Devoto",
    streak_14_desc: "Sequência de 14 dias no desafio diário",
    daily_first: "Jogador Diário",
    daily_first_desc: "Complete um desafio diário",
    games_10: "Frequente",
    games_10_desc: "Jogue 10 partidas",
    games_50: "Veterano",
    games_50_desc: "Jogue 50 partidas",
    games_100: "Centurião",
    games_100_desc: "Jogue 100 partidas",
  },
  review: {
    title: "Gostando do Eco Mi?",
    subtitle: "Sua opinião nos ajuda a melhorar!",
    loveIt: "Adoro!",
    notReally: "Nem tanto",
    maybeLater: "Talvez Depois",
  },
  errorScreen: {
    title: "Algo deu errado!",
    friendlySubtitle: "Ocorreu um erro inesperado. Por favor, tente reiniciar o aplicativo.",
    reset: "REINICIAR APP",
  },
  tracking: {
    title: "Um aviso rápido",
    subtitle:
      "Na próxima tela, o iOS perguntará se o Eco Mi pode exibir anúncios personalizados. Você pode permitir ou negar; o jogo funciona da mesma forma nos dois casos.",
    continueButton: "Continuar",
  },
  onboarding: {
    tapHint: "Toque no botão que acendeu!",
  },
  notifications: {
    dailyReminder: "Hora do seu desafio de memória diário!",
    streakAtRisk: "Não perca sua sequência de {{count}} dias! Jogue hoje.",
    winBack: "Sentimos sua falta! Suas habilidades de memória estão enferrujando.",
    prePromptTitle: "Não perca nenhum momento",
    prePromptBody:
      "Receba lembretes de desafios diários, alertas para proteger sua sequência e incentivos para manter sua memória afiada.",
    enableReminders: "Ativar lembretes",
    maybeLater: "Talvez mais tarde",
  },
  settings: {
    soundEnabled: "Ativado",
    volume: "Volume",
    haptics: "Hápticos",
    hapticsToggle: "Vibração",
    notifications: "Notificações",
    notifyDaily: "Lembretes de desafio diário",
    notifyStreak: "Alertas de proteção de sequência",
    notifyWinback: "Lembretes de retorno",
    accessibility: "Acessibilidade",
    colorblindPatterns: "Padrões de cor",
    colorblindPatternsHint: "Mostrar formas em cada botão para jogadores daltônicos",
  },
  iap: {
    postPBTitle: "Novo Recorde Pessoal!",
    postPBBody: "Desfrute de uma experiência sem anúncios",
    maybeLater: "Talvez Depois",
    tiredOfAdsTitle: "Curtindo Eco Mi? Jogue sem anúncios.",
    tiredOfAdsBody: "Compra única. Sem assinaturas.",
    removeAdsCta: "Remover anúncios",
    skipAdsCta: "Sem anúncios",
  },
  share: {
    errorTitle: "Falha ao compartilhar",
    errorBody: "Não conseguimos abrir o menu de compartilhamento. Tente novamente.",
  },
  a11y: {
    modeSelector: "Selecionar modo",
    settings: "Configurações",
    leaderboard: "Placar",
    stats: "Estatísticas",
    achievements: "Conquistas",
    startGame: "Iniciar jogo",
    endGame: "Encerrar jogo",
    soundToggle: "Ativar/desativar som",
    restorePurchases: "Restaurar compras",
    removeAds: "Remover anúncios",
    padLabel: "Botão {{color}}, {{position}}",
    padColor_red: "vermelho",
    padColor_blue: "azul",
    padColor_green: "verde",
    padColor_yellow: "amarelo",
    padPositionTopLeft: "canto superior esquerdo",
    padPositionTopRight: "canto superior direito",
    padPositionBottomLeft: "canto inferior esquerdo",
    padPositionBottomRight: "canto inferior direito",
    applyTheme: "Aplicar tema",
    previewLockedTheme: "Pré-visualizar tema bloqueado",
    applySoundPack: "Aplicar pacote de som",
    previewLockedSoundPack: "Pré-visualizar pacote bloqueado",
    watchSequence: "Observe a sequência: {{pads}}. Repita quando estiver pronto.",
    gameOver: "Fim de jogo. Pontos {{score}}, nível {{level}}.",
  },
  emptyStateComponent: {
    generic: {
      heading: "Nada por aqui...",
      content: "Nenhum dado encontrado. Tente pressionar o botão para atualizar.",
      button: "Tentar novamente",
    },
  },
  themes: {
    classic: "Clássico",
    neon: "Néon",
    retro: "Retrô",
    pastel: "Pastel",
  },
}

export default pt
