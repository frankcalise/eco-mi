...
function loadHapticsEnabled(): boolean {
  return loadString(SETTINGS_HAPTICS_ENABLED) !== "false"
}

/**
 * Reactive source of truth for user preference flags. Settings screen writes
 * through this store so other parts of the app (e.g. useHaptics) re-render
 * immediately when a toggle flips — MMKV-only reads snapshot at mount and
 * miss in-session changes.
 */
